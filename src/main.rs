...
fn main() {
    ...
    game.window_settings(Window {
        title: "Rusty Tutorial".to_string(),
        ..Default::default()
    });
    ...
    score.translation = Vec2::new(500.0, 300.0);

    let high_score = game.add_text("high_score", "High Score: 0");
    high_score.translation = Vec2::new(-500.0, 300.0);
...
fn game_logic(engine: &mut Engine, game_state: &mut GameState) {
    ...
    if game_state.spawn_timer.tick(engine.delta).just_finished() {
        ...
        game_state.ferris_index += 1;
        let ferris = engine.add_sprite(label.clone(), SpritePreset::RacingCarYellow);
        ferris.translation.x = random_range(-500.0..500.0);
        ferris.translation.y = random_range(-300.0..300.0);
        ferris.collision = true;
    }
    // Reset score
